...
- **步骤 2.5 为 skill 的唯一硬停点**：新建页面 / 新建项目任务默认进入 2.5 节点。
  - **有交互能力的运行时**（Cursor / Claude Code / Messenger MCP 等）：**必须**通过 `ask_question` 弹出 3 个推荐品牌 + 第 4 项"不使用品牌"兜底选项，**禁止**静默默认。
  - **无交互能力的运行时**（CI / headless / auto-approve 等）：允许降级为 AntD 默认，**但必须**在输出头部以 banner 显式声明本次为非交互模式，并提示如何指定品牌。
  - 用户显式说"按默认 / B 端标准"或增量修改任务 → 自动跳过 2.5，不带 banner。
  - 用户选"不使用"即走默认样式。业务领域词（用户管理 / 订单管理 / CRUD 等）**不等同**跳过短语。
...
| 路径 | 触发条件 | Agent 行为 |
|------|---------|-----------|
| 🟢 **默认交互推荐**（有交互能力时走此路径） | 新建页面 / 项目 + 无显式品牌 + 未显式跳过 + 运行时有 `ask_question` | **必须**通过 `ask_question` 推荐 3 个品牌 + "不使用品牌" 选项，用户一键选择 |
| ⚫ **非交互降级** | 同 🟢 条件但运行时无交互工具（CI / headless / auto-approve） | 降级走 AntD 默认，**但必须**在输出头部 banner 声明"本次为非交互模式"，提示如何指定品牌 |
| 🔵 **显式指定品牌** | _"用 Linear 风格做 XX"_ | 跳过推荐对话，直接加载对应 DESIGN.md，生成主题层 |
| 🟡 **用户显式跳过** | _"按默认 / B 端标准 / 不用设计系统"_ | 自动跳过 2.5，走现状 AntD 默认，和原工作流 100% 等同，**不带 banner** |
...
**P0 级违规**（会在步骤 5 代码审查中被标记）：
- ❌ 有交互能力却没弹 `ask_question`，直接默认 AntD
- ❌ 弹了但第 4 项"不使用品牌"缺失
- ❌ 非交互降级时未在输出头部 banner 声明

> **"严格按 skill 执行" ≠ 自动一路到底**：硬停点必须照走，无法交互时必须显式声明。业务领域词（用户管理 / 订单管理 / CRUD 等）不是跳过短语。
