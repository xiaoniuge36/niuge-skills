...
1. **步骤 1 必须先完成**：未检测环境不得生成业务代码；无项目须先确认再创建。**新项目默认 React + TypeScript + Ant Design Pro**。
   - **企业管理后台首选脚手架**：若用户原话包含「管理后台 / 后台管理 / admin / 中后台 / 企业后台 / 运营平台 / 管理系统 / CRM / 工单 / 控制台」之一，且目标目录无 `package.json` → **默认使用 [`create-admin-platform`](https://www.npmjs.com/package/create-admin-platform) CLI**（React 19 + AntD 6 + React Router 7 + Vite 8，内置登录/权限/主题/MSW mock）。
     - 标准命令（**不带 `--thsk`**）：
       ```bash
       ...
       pnpm dlx create-admin-platform@latest <project-name> \
         ...
         --theme default
       ```
     - 用户明确说"最小化 / 只要空 Vite / 不要登录权限 / 不要 mock"才退化为裸 Vite + react-ts。
     - 详细 flag 说明与决策表见 [references/environment-setup.md](references/environment-setup.md) 的「脚手架选择决策表」与「React 企业管理后台项目」小节。
...
3. **步骤 2.5 设计推荐（唯一硬停点，三条契约）**：位于需求分析后、模板匹配前。完整规则见 [references/design-md-integration.md](references/design-md-integration.md)。

   **契约 1 · 是否触发**：
   任务类型属于"新建页面 / 新建项目 / 新增页面"**且**用户原话**未**显式包含以下跳过短语之一 → 「按默认风格」「B 端标准」「不用设计系统」「按 AntD 默认」「按组件库风格」→ **必须进入本节点**。

   > 注：业务领域词"用户管理 / 订单管理 / 权限管理 / 列表页 / 表单页 / CRUD" **不是**跳过短语，不触发跳过。

   **契约 2 · 交互能力分支（核心）**：
   - 运行时**有 `ask_question` 类交互工具**（Cursor / Claude Code / Messenger MCP 等）→ **必须**通过 `ask_question` 弹出 3 个推荐品牌 + 第 4 项「不使用品牌（默认 Ant Design 样式）」让用户选。**第 4 项永远存在**。**禁止**以任何理由静默默认。
   - 运行时**无交互工具**（CI / headless / auto-approve 模式等）→ 允许降级为 `brand=none`（AntD 默认），**但必须**在最终生成结果的开头（即 Agent 聊天输出开头，非源码文件）以 banner 显式声明：

     ```
     ⚠️ 本次为非交互模式，已默认使用 Ant Design 无品牌主题。
        如需指定风格请重新运行并加："用 linear 风格做 XXX"。
        可选品牌见 https://getdesign.md/ 或 references/design-systems/index.json。
     ```

   **契约 3 · 强触发直跳与显式跳过（沿用原规则）**：
   - 用户显式品牌（_"用 linear 风格"_）或本地 DESIGN.md 路径 → **跳过推荐对话**，直接进入步骤 3.5。
   - 用户原话含跳过短语（见契约 1）→ **跳过本节点**，直接进入步骤 3。
   - 增量修改任务（修改已有页面、调整字段等）→ **默认跳过推荐**，除非用户明确说换主题。
   - 用户选第 4 项 / 超时不回应 / 降级默认 → `selectedBrandId = null`，**跳过** 3.5 / 4 主题层 / 5 主题审查。

   **P0 级违规**（等同 P0 错误，步骤 5 审查必查）：
   - ❌ Agent 有交互能力却没弹 `ask_question`，直接默认 AntD
   - ❌ 弹了但第 4 项「不使用品牌」缺失
   - ❌ 降级默认时未在输出头部 banner 声明

   **加载方式**：
   - **远程优先**：首选从 `https://getdesign.md/` 实时抓取最新品牌列表并覆盖 `references/design-systems/index.json`。
   - **本地降级**：远程抓取失败时回退读本地 `index.json`，并向用户输出 warning 提示使用本地缓存。
   - **远程与本地都失败**：按"契约 2 · 无交互工具"路径降级（带 banner），不阻塞主流程。

   **推荐产出规则**：Agent 基于步骤 2 抽取的风格信号（品牌名 / 本地路径 / 风格词 / 无信号 + 需求类型）从当次加载到的 `brands[]` 中选出最匹配的 3 个品牌。**禁止幻觉**：推荐的品牌 id 必须来自 `brands[]`，不得推荐未在索引中的品牌。**任务内单次抓取**：同一生成任务内已成功抓过索引，后续复用当次结果。
...
| `frontend-code-spec` | 规范合规参考 | 用户要求前端代码规范、f2elint 或项目已接入 ali config 时，用于 JS/TS/React/Node/CSS/HTML 与工程规约审查（基于 alibaba/f2e-spec） |
...
| 用户输入 | 动作 |
|----------|------|
| 「严格按 skill 执行 …」 / 「严格按约束 …」 | **严格 ≠ 自动**。所有硬停点照走：新项目按伪代码询问技术栈；2.5 **必须**弹 `ask_question` 选项；若运行时无交互能力，**必须**在输出头部 banner 声明降级，不得静默默认 |
...
| 「创建管理后台项目」 / 「创建 admin 项目」 / 「搭一个后台管理系统」 / 「新建中后台」 | 步骤 1 → 走 `create-admin-platform` 默认命令（不带 `--thsk`）：`pnpm dlx create-admin-platform@latest <name>` |
| 「创建管理后台并用 linear 风格」 | 步骤 1 → `pnpm dlx create-admin-platform@latest <name> --theme linear` |
| 「创建管理后台，不要 mock 后端」 | 步骤 1 → `pnpm dlx create-admin-platform@latest <name> --features=admin,security,form-builder` |
| 「创建管理后台，用 browser 路由」 | 步骤 1 → `pnpm dlx create-admin-platform@latest <name> --router-mode browser` |
...
- 新建**管理后台**类项目默认使用 `create-admin-platform` CLI（OSS 默认模式，**不带 `--thsk`**）；非管理后台或用户显式要求最小化时才退化为裸 Vite。详见 [references/environment-setup.md](references/environment-setup.md) 的「脚手架选择决策表」。
...
- **2.5 是 skill 中的唯一硬停点**：在有交互能力的运行时，2.5 **必须**弹出 3+1 `ask_question` 选项；在无交互能力的运行时，**必须**以 banner 在输出头部显式声明降级。**禁止静默默认**。违反此条等同 P0 错误，将在步骤 5 审查中被标记。
