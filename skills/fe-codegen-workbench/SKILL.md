...
| 用户提供原型图、设计稿、品牌视觉稿 | 在步骤 2-4 **优先加载** `frontend-design`，用于补强视觉方向、排版、配色和交互 |
| 用户明确要求“做得更高级”“重做体验”“高保真还原 UI/UX” | 若本地已安装可信来源的 `ui-ux-pro-max`，可在 `frontend-design` 之后作为补充参考 |
| 已有成熟设计系统/组件库约束 | 设计类 Skills 只能做增强参考，**不能覆盖** 业务组件约束、字段规范和现有设计系统 |

**结论**：`frontend-design` 适合做 `fe-codegen-workbench` 的条件化设计增强；`ui-ux-pro-max` 适合作为可选增强，不建议升级为强依赖或默认强制链路。
...
4. **步骤 4 必须加载技术栈知识库**：React 项目加载 `react-antdpro-knowledge.md`，Vue 项目加载 `vue-knowledge.md`。严格遵守 `code-standards.md`：hooks 优先、全局类型不 import、禁止 mock（包括 localStorage/内存模拟）、文件生成顺序不可乱。**ProTable 必须使用 request 模式，搜索栏必须使用内置 search，状态列必须使用 valueEnum。**
...
| 原型图 + 描述 | 步骤 2 加强（优先加载 frontend-design；若已安装 `ui-ux-pro-max` 可继续补强），再走 3–5 |
...
- 标准企业级页面默认优先复用现有设计系统；只有在视觉风格和体验质量是显性目标时，才调度设计类 Skills。
