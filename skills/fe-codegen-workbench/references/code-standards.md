...
| 检查项 | 检查内容 | 通过标准 |
|--------|---------|----------|
| 类型引入 | 是否重复引入全局类型 | 全局类型不应被 import |
...
| **ProTable request** | 是否使用 request 而非 dataSource | 必须使用 request 模式 |
| **ProTable search** | 是否使用内置 search 而非手动 Form | 必须使用 search 属性 |
| **过度设计** | 是否添加了用户未要求的模块 | 禁止统计卡片、装饰标签等 |
| **mock 数据** | 是否用 localStorage/内存模拟接口 | 必须使用真实 API 调用骨架 |
| **视觉风格** | 是否使用 Ant Design 默认样式 | 禁止自定义渐变、阴影、字体 |
| **Service 层** | 是否生成了 API 调用文件 | 无现成接口时必须生成 |
