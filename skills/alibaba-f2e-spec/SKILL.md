...
Use Alibaba's `alibaba/f2e-spec` as a practical engineering standard, not as style trivia. Prefer automated tooling first, then use the human-readable spec to guide code generation, review, refactoring, and exceptions.
...
2. Load [references/spec-index.md](references/spec-index.md) and pick only the relevant spec areas.
3. If the user asks to install or standardize tooling, load [references/tool-adoption.md](references/tool-adoption.md).
4. If generating or reviewing code, load [references/review-checklist.md](references/review-checklist.md).
5. Prefer the repo's existing conventions when they are stricter or project-specific. Do not replace established local rules unless the user asked for Alibaba f2e-spec alignment.
6. Run the smallest relevant verification command available in the project. If tooling is not installed, report the exact missing tool and the recommended next command.
